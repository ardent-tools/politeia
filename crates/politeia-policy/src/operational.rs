...
mod qualification_tests {
    use super::{QualificationVector, qualification_replay_domain};
    use politeia_core::{Digest, RuntimeGenerationId};

    #[test]
    fn qualification_replay_scope_survives_capability_readmission() {
        let candidate = RuntimeGenerationId::derive(b"candidate");
        let policy = Digest::blake3(b"policy");
        let first = qualification_replay_domain(
            &candidate,
            &policy,
            "binding",
            "control",
        ...
        .expect("semantic replay scope encodes");

        // Admission time, revision, active pointer, grant, and reserved run ID
        // cannot affect this call. A later capability for the same semantic
        // target therefore reaches the same durable replay namespace.
        let readmitted = qualification_replay_domain(
            &candidate,
            &policy,
            "binding",
            "control",
        ...
        .expect("readmitted semantic replay scope encodes");
        assert_eq!(first, readmitted);
        assert_ne!(
            first,
            qualification_replay_domain(
                &candidate,
                &policy,
                "binding",
                "control",
            ...
            .expect("the planted vector scope encodes")
        );
    }
}
