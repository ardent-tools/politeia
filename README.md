...
Activation and rollback use `GenerationRequest::Transition`: the installed owner signs the action, target generation, expected workspace revision and active generation, and the exact independent assurance digest. The daemon retains that signed decision with the transition. Operational generations must approve the executable image actually running the daemon; deterministic resource descriptors bind that same executable.
...
Operational handoff uses `CommissioningRequest::Handoff`. After commissioner authority ends, the owner signs revocation evidence, runs a fresh governed canary, and signs continuity evidence bound to its completed receipt. The daemon checks the durable authority and operation records, then atomically preserves a canonical `HandoffReceipt` with both signed evidence wires. A handoff canary needs a retained semantic replay identity: operations that do not require an idempotency key may still supply one; a keyless ordinary request is valid but cannot certify handoff. Replacement commissioning requires a fresh owner grant.
...
The script uses `POLITEIA_STORAGE_TEST_DATABASE_URL` when supplied, or starts a disposable PostgreSQL container with Docker or Podman. Use an isolated or disposable database: the fixture changes schema and installs test triggers. PostgreSQL is required: an absent database is a failed prerequisite, never a passing acceptance result. Local runs retain each observation in a new file under `target/package-acceptance`; set `POLITEIA_ACCEPTANCE_ARTIFACT_DIR` to choose another directory. CI custody uses `POLITEIA_ACCEPTANCE_ARTIFACT_FILE` for one immutable package observation.
